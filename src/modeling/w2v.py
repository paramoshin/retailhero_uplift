...
if __name__ == "__main__":
    ...
    model = Word2Vec(
        min_count=1, workers=6, window=5, sg=1, hs = 0, negative = 10, alpha=0.03, min_alpha=0.0007, seed=42
    )
    model.build_vocab(data["products_list"].tolist(), progress_per=200)
    model.train(data["products_list"].tolist(), total_examples=model.corpus_count, epochs=10, report_delay=1)
