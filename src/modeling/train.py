...
if __name__ == "__main__":
    ...
    for rs in folds.columns:
        ...
        for i in range(folds[rs].nunique()):
            ...
            (
                ...
                y_valid_control,
                y_valid_treatment,
            ) = split_control_treatment(test_data, test_target, test_data_is_treatment)

            if args.model == "xgb" and args.use_best:
                clf_control = xgb.XGBClassifier(
                    objective="binary:logistic", **control_best_params
                ).fit(X_train_control, y_train_control)
                clf_treatment = xgb.XGBClassifier(
                    objective="binary:logistic", **treatment_best_params
            ...
            log_artifact(
                (folder / Path(model_name + "_control.pkl")).resolve(), f"control-clf-fold-{i}"
            )
            log_artifact(
                (folder / Path(model_name + "_treatment.pkl")).resolve(), f"treatment-clf-fold-{i}"
            )
    ...
    if args.refit:
        ...
        if args.model == "xgb":
            clf_control = xgb.XGBClassifier(objective="binary:logistic", **control_best_params).fit(
        ...
        joblib.dump(
            clf_control, Path(f"../../models/two_models/{args.model}_refit_control.pkl").resolve()
        )
        joblib.dump(
            clf_treatment,
            Path(f"../../models/two_models/{args.model}_refit_treatment.pkl").resolve(),
        )
        log_artifact(
            Path(f"../../models/two_models/{args.model}_refit_control.pkl").resolve(),
            f"control-clf-fold-{i}",
        )
        log_artifact(
            Path(f"../../models/two_models/{args.model}_refit_treatment.pkl").resolve(),
            f"treatment-clf-fold-{i}",
        )
        ...
        if args.platts_scaling:
            ...
            lr_treatment = LogisticRegression().fit(
                clf_treatment.predict_proba(X_train_treatment)[:, 1].reshape(-1, 1),
                y_train_treatment,
            )
