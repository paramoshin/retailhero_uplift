...
if __name__ == "__main__":
    ...
    if args.category == "level_2":
        ...
        for c_id, c in purchases_client_ids.iterrows():
            ...
            frame = pd.read_csv(
                "../../notebooks/purchases_level_2.csv",
                skiprows=[i for i in range(1, skip + 1)],
                nrows=c[0],
            )
            data = (
                frame.groupby("client_id")[[x for x in frame.columns if x.startswith("level_")]]
                .sum()
                .reset_index()
                .groupby("client_id")
        ...
        for c_id, c in purchases_client_ids.iterrows():
            ...
            if i % 5000 == 0:
                print(f"{i} of {purchases_client_ids.shape[0]}")
            frame = pd.read_csv(
                "../../notebooks/purchases_segment.csv",
                skiprows=[i for i in range(1, skip + 1)],
                nrows=c[0],
            )
            data = (
